...
export const useFileTreeStore = defineStore('fileTree', {
  ...
  actions: {
    ...
    async scan () {
      ...
      await Promise.all(
        Object.entries(targetChildren).map(async ([key, child]) => {
    ...
    //前序懒加载
    async preOrderLazyLoad () {
      const removedItems = this.result.splice(0, 3)
      //难点在于触发响应式更新，这点不熟悉，要给this创建新引用
      this.result = [...this.result]
      if(this.preLazyPathArr.length > 0){
        this.preLazyPathArr = [...this.preLazyPathArr, ...removedItems]
      }else{
        this.preLazyPathArr = removedItems
      }
    },
    async preLazyLoad () {
      if (this.preLazyPathArr.length > 0) {
        this.result = [...this.preLazyPathArr, ...this.result]
        this.preLazyPathArr = []
      }
    },
    ...
    //批量下载
    async batchDownload () {
      const resultArray = Object.keys(this.checked).map(
        key =>
          `${this.routeArr.slice(2).join('/') ? this.routeArr.slice(2).join('/') + '/' : ''}${key}`
      )
      console.log(resultArray)
      await apiClient
        .post('/files/ForDescribe', {
