      ...
      <el-scrollbar class="preview-content"
        ...
        <div class="file-viewer-container">
          ...
          <div class="editor-section">
            <div v-html="text" class="editor"></div>
          </div>
          <!-- 未知类型提示 -->
...
marked.setOptions({
  ...
  langPrefix: 'hljs language-', // 用于CSS类名前缀
...
async function renderLargeMarkdown(url) {
  try {
    // 分块加载大文件
...
const fetchFileContent = async () => {
  try {
    ...
    switch (currentFileType.value) {
      case 'md':
        renderLargeMarkdown(response.data.previewURL)
        break
      default:
