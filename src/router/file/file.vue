<script setup>
import { reactive, ref, onMounted, toRaw, computed, watch, onUpdated, onBeforeUpdate, nextTick, onUnmounted } from 'vue';
...
watch(
  ...
  () => {
    ...
    if (!dialogVisible.value) { check.value = {} }
  }
)
...
const downloadFile = async (item) => {
  try {
    if (route.query.type != 'file') {
      ...
      // 创建下载链接
      const url = window.URL.createObjectURL(new Blob([response.data.previewURL]));
...
<template>
  <Move :dialogVisible="dialogVisible" @update:dialog-visible="val => dialogVisible = val" :check="check" :type="type"
    @move-control="handleMoveControl" v-model:dialog-visible="dialogVisible" />
  <el-main @scroll="throttledScroll">
    <el-header height="40px">
      <p class="text" v-if="query.type != 'file' || toRaw(levelStore.level).length === 0">
        全部{{ query.type }}
      </p>
      <tree :type="query.type" v-if="query.type == 'file' && toRaw(levelStore.level).length > 0" />
      <div class="file-list-header">
        <span v-for="(item, index) in columns" :key="item.key" @click="sortList(item.key)">{{ item.title }}
          <el-icon>
    ...
    <div class="file-list" v-loading="loading" element-loading-text="Loading..." element-loading-custom-class="loading">
      <TransitionGroup name="list" tag="p">
        <div class="file-q" v-if="empty">
          <el-empty description="暂无文件" :image="simpleImage" image-size="150" />
        </div>
        <div class="file-list-body" v-for="(item, index) in fileTreeStore.result" :key="index"
          @mouseover='getDivDom(item)' @mouseout="nogetDivDom(item)">
          <!-- :fit="fit" -->
          <p v-show="!divDomList[item.name] && editingFileName !== item.name"> <el-image
              style="width: 25px; height: 25px" :src="url(item)" /><span>{{
                item.name }}</span> <span>{{ item.size }} </span><span>{{
                item.date }}</span></p>
          ...
          <Transition name="fade" tag="div">
            ...
            <div v-show="editingFileName === item.name" class="edit-mode">
              <el-input v-model="tempName" ref="editInput" size="small" style="width: 240px">
              </el-input>
              <div class="edit-but">
                <el-button size="small" @click="submitRename(tempName, item)" plain type="primary"> <!-- 紧凑按钮 -->
                  √
                </el-button>
                <el-button size="small" @click="cancelEdit" plain type="primary"> <!-- 紧凑按钮 -->
                  X
                </el-button>
              </div>
            </div>
          ...
          <div v-show="divDomList[item.name] && editingFileName !== item.name" :class="{ active: check[item.name] }"
            class="checkbox">
            <el-checkbox :v-model="check[item.name]" size="small" label="   " @click="checked(item.name)" /><span
              id="text" @click="enter(item)">{{
                item.name }}</span>
            <el-icon>
              <Link />
            </el-icon>
            ...
            <el-popconfirm confirm-button-text="执行" cancel-button-text="取消" icon-color="#f90" title="是否要执行删除文件夹"
              @confirm="confirmEvent(item)" :teleported="false">
            ...
            <div style="color: #27ba9b;height: 100%;display: flex;align-items: center;">
              <el-dropdown style="margin-left: 15px;" :teleported="false">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item @click="MoveControl(item.name, 'copy')"><el-icon>
                        <DocumentCopy />
                      </el-icon>复制</el-dropdown-item>
                    <el-dropdown-item @click="MoveControl(item.name)"><el-icon>
                        <Rank />
                      </el-icon>移动</el-dropdown-item>
                    <el-dropdown-item><el-icon>
                        <FolderAdd />
                      </el-icon>导出项目目录</el-dropdown-item>
                    <el-dropdown-item><el-icon>
                        <Place />
                      </el-icon>共享</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </div>
